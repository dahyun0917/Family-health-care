                            ...
                            VStack{
                                ForEach(fam.users,id:\.userId){ user in
                                    ForEach(user.promise.indices){ index in
                                        let promise = user.promise[Int(index)]
                                        NavigationLink(destination: Text("상세 페이지")){
                                            PromiseView(text:promise.promiseDetail,date:promise.dateFormatted(),complete:promise.isCompleted,option:-1,color:index%2 == 0 ?Color.mainBeige:Color.mainBlue,user:user)
